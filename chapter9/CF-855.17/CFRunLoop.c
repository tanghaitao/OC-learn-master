...
struct __CFRunLoopMode {
    ...
    CFStringRef _name;//字符串类型 对应的是某一个runloop它的nsdefaultrunloopmode默认mode的名称，可以通过name找到对应的mode，所以commonmodes 中
    //存储的是不同模式的名称，这个设计思想是怎么样的？
    Boolean _stopped;
    char _padding[3];
    CFMutableSetRef _sources0;//和runloop包含的commonmodeitems存储的元素是不一样的，末尾是代个s的。，是集合类型的数据结构 ， 无序
    CFMutableSetRef _sources1;//同上
    CFMutableArrayRef _observers;//是有序的，集合是无序的
    CFMutableArrayRef _timers;//同上
...
struct __CFRunLoop {
    ...
    pthread_t _pthread;// c语言的线程对象:代表线程，runloop和线程是一一对应的关系
    uint32_t _winthread;
    CFMutableSetRef _commonModes;//是一个集合 nsmutableset<nsstring*>,代表common模式的集合，为什么是字符串？
    CFMutableSetRef _commonModeItems;//是一个集合，集合中包含多个元素，包含多个 observer观察者（可为runloop添加观察者） 多个timer 多个source（可以提交到对应的某个runloop对应的某个runloopmode上面）
    CFRunLoopModeRef _currentMode;//runloop当前所处的一个模式：是一个cfrunloopmode的数据结构，一会可以对其进行讲解
    CFMutableSetRef _modes;//runloop多个mode的一个集合，一对多的关系：是一个集合 nsmutableset<cfrunllopmode*>
