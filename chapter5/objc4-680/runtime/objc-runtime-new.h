...
struct class_ro_t {
    ...
    const char * name;//类名称
    method_list_t * baseMethodList;// 只读的方法列表
    protocol_list_t * baseProtocols;// 只读的协议列表
    const ivar_list_t * ivars;// 只读的变量列表

    const uint8_t * weakIvarLayout;
    property_list_t *baseProperties;// 只读的属性列表
...
struct objc_class : objc_object {
    // Class ISA;
    /*
     如果objc_class是类对象，superClass那指向的是父类对象，平时的类与父类的关系是通过
     superClass去定义的。
     */
    Class superclass;
    cache_t cache;//表达方法缓存的，在消息传递过程中，会使用方法缓存的数据结构
    //一个类所定义的一些变量属性包括它的一些方法都在bits它的成员结构中，主要是对class_rw_t的封装。
