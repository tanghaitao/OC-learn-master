...
void _object_set_associative_reference(id object, void *key, id value, uintptr_t policy) {
    ...
    {
        AssociationsManager manager; //关联对象管理类，c++实现一个类
        //获取其维护的一个associationshashmap,我们可以理解为是一个字典
        //是一个全局容器
        AssociationsHashMap &associations(manager.associations());
        disguised_ptr_t disguised_object = DISGUISE(object);//指针地址按位取反，作为全局容器的key
        if (new_value) {
            ...
            } else {//全局容器第一次添加该类的关联对象
                // create the new association (first time).
                ObjectAssociationMap *refs = new ObjectAssociationMap;
                //AssociationsHashMap封装 key(DISGUISE(object)) 和 value(ObjectAssociationMap)
                associations[disguised_object] = refs;
                //ObjectAssociationMap封装 key(key) 和 value(ObjcAssociation)
                (*refs)[key] = ObjcAssociation(policy, new_value);
                object->setHasAssociatedObjects();//有了关联对象
            }
        } else {
            ...
            if (i !=  associations.end()) {
                ObjectAssociationMap *refs = i->second;
                ObjectAssociationMap::iterator j = refs->find(key);
                if (j != refs->end()) {// value=nil已经有关联对象的擦除操作
                    old_association = j->second;
                    refs->erase(j);
                }
            }
